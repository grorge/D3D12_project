...
void Renderer::tm_runFrame(const unsigned int iD)
{
	...
	while (this->running)
	{
		...
		if (myTurn[iD] && this->frameReadyToClear[iD])
		{
			...
			//Legacy stuff, could be done with only one. Kept so that we could change to clear another texture.
			...
			if (RUN_TIME_STAMPS)
			{
				...
				this->ClearTexture(m_graphicsCmdList[iD](), iD);				//Copy Translation  
...
void Renderer::tm_copy()
{
	while (this->running)
	{
		if (this->input < LOGIC_LIMIT || NO_LOGIC_LIMIT)
		{
...
void Renderer::tm_runCS()
{
	while (this->running)
	{
		if (this->logic < LOGIC_LIMIT || NO_LOGIC_LIMIT)
		{
			this->WaitForCompute();
